...
We used a logistic regression to model the probability that an animal at the Long Beach Animal Shelter is adopted. The final model was selected using forward stepwise AIC reduction and includes the predictors: animal_type, intake_condition, sex, and intake_type.

Null deviance: 30,621 on 29,599 degrees of freedom
Residual deviance: 21,683 on 29,559 degrees of freedom
AIC: 21,765, representing lots of improvement from the null model AIC of 30,623.21

The final model is:
logit(p_i) = β0 + β1*animal_type + β2*intake_condition + β3*sex + β4*intake_type

Baseline: Normally behaved, male, foster, dog
...
Key Findings per Predictor

## Animal Type
Animal Type | Estimate | Odds Ratio | p-value
Bird | 1.044 | 2.84× | 7.27e-07
Cat | 0.142 | 1.15× | 0.0004
Guinea pig | 1.797 | 6.03× | < 2e-16
Livestock | 2.574 | 13.11× | 0.0001
Rabbit | –0.579 | 0.56× | 6.9e-08

Animal type significantly affects adoption likelihood. Livestock and guinea pigs had the highest odds; rabbits had reduced odds compared to dogs.

## Intake Condition
Condition | Estimate | Odds Ratio | p-value
Feral | –2.693 | 0.07× | < 2e-16
Fractious (aggr.) | –1.699 | 0.18× | < 2e-16
Behavior severe | –1.189 | 0.30× | 0.0055
Ill severe | –0.875 | 0.42× | < 1e-9
Injured severe | –1.237 | 0.29× | < 2e-16
Under age/weight | 0.220 | 1.25× | 3.24e-05

Animals in poor health or with behavioral concerns have drastically lower adoption odds. Feral and fractious animals have 80–93% lower odds of adoption.

## Sex

Category | Estimate | Odds Ratio | p-value
Neutered | 2.407 | 11.10× | < 2e-16
Spayed | 2.430 | 11.36× | < 2e-16
Unknown | –1.871 | 0.15× | < 2e-16

Sterilization is the most impactful predictor. Spayed/neutered animals are over 11× more likely to be adopted than intact males.

## Intake Type
Intake Type | Estimate | Odds Ratio | p-value
Trap-Neuter-Return | –3.601 | 0.027× | 0.023

TNR animals are not meant to be adopted, and their extremely low adoption odds validate excluding them from adoption success metrics.

## Looking at the peason residual models

Model Diagnostics: Pearson Residuals
We examined Pearson residuals versus fitted values for each predictor included in the final logistic regression model in order to assess model fit.
1. Animal Type
...
Conclusion: The model performs well overall.

...
$$
\begin{aligned}
  \mathrm{logit}(p_i) = \beta_0 + \beta_1 \cdot I(\text{animal\_type}_i)\\
+ \beta_2 \cdot I(\text{intake\_condition}_i) \\
+ \beta_3 \cdot I(\text{sex}_i) + \beta_4 \cdot I(\text{intake\_type}_i)
\end{aligned}
$$

where I indicates a series of dummy variables: animal_type = (bird, cat, livestock, etc.), intake_condition = (aged, behavior mild, etc.), sex = (female, spayed, neutered), and intake_type = (foster, stray, etc.) where those variables take value 1 when the condition is true and 0 otherwise. (beta_0 to beta_39).
...
- Other predictors also lowered the AIC: intake_type: AIC = 25533.36, animal_type: AIC = 26776.34, intake_condition: AIC = 29932.68
...
- Adding other variables instead gives higher AICs: intake_type: AIC = 22616.49, animal_type: AIC = 22769.29
...
- Other options: animal_type: AIC = 22021.64, intake_type is selected as the next best variable.
...
This shows the estimated probability of adoption by animal type.
    - Dogs and cats have moderate probabilities of adoption (~0.5–0.6). Birds, rabbits, and guinea pigs show higher probabilities. Livestock and reptiles also display high estimated probabilities, but these results may be due to low sample size or outlier cases (like adoptions through special programs).
    
- Residual Plot:
The deviance residuals mostly fall within ±2, indicating good model fit across animal types. A few larger residuals are visible for less common types like wild or reptile, suggesting predictions for those types might be less accurate (less data). The model captures adoption trends well for major categories. Some rare animal types may need further data.

 Intake Condition 
 
- Probability Plot: 
Shows a clear trend: “Normal”, “under age/weight”, and “ill (mild)” animals have higher adoption probabilities. Severely ill, injured, feral, or fractious animals show much lower probabilities. Behavioral concerns—especially severe—lower adoption chances.
    
- Residual Plot: 
Most values fall within ±2, suggesting overall good model fit. Slight under- or overfitting is seen in categories with smaller sample sizes like “welfare seizures” and “i/i report”. Intake condition is a strong predictor of adoption. Predictions for rare or ambiguous categories (like “i/i report”) might be less reliable.

 Intake Type 

- Probability Plot: 
Animals from foster, owner surrender, and stray categories have higher adoption probabilities. Trap-Neuter-Return (TNR) and wildlife have very low adoption probabilities—as expected since they are often returned, not adopted.
    
- Residual Plot: 
Deviance residuals mostly within ±2 again, showing decent fit. Some underprediction may occur for “stray” and “welfare seized,” possibly due to mixed adoption outcomes in those groups.The model fits well, and the adoption probabilities align with operational shelter expectations. Intake type strongly influences adoption likelihood.

 Sex 
 
- Probability Plot:
Spayed and neutered animals show significantly higher adoption probabilities. Unaltered males and females are less likely to be adopted.
    
- Residual Plot: 
Residuals are mostly near zero, indicating excellent model fit. Sterilization status is one of the most important predictors.
